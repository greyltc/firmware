...
void uStepperDriver::reset( void ){
	...
	this->writeRegister( PWMCONF, 	0x00050480 );	
	...
	this->writeRegister( DRV_STATUS,0 );
	this->writeRegister( RAMP_STAT, 0 );

	this->writeRegister( SW_MODE, 	0 );
	this->writeRegister( COOLCONF, 	0 );
	this->writeRegister( TSTEP, 	0 );
...
void uStepperDriver::init( uStepperS * _pointer ){
	...
	/* Setup encoder */
	this->writeRegister( ENCMODE, 0x00);
	//this->writeRegister( ENC_CONST, 0x00FFFFFF);
	this->writeRegister( ENC_CONST, (int32_t)-26214399); // this sets the encoder to ~match (within 1 microstep) the stepper
	...
	this->writeRegister( CHOPCONF, DEDGE(0) | INTPOL(0) | MRES(0) | TOFF(2) | TBL(2) | HSTRT_TFD(4) | HEND(0) | SYNC(0) | RNDTF(0) | CHM(1));
	...
	//this->writeRegister( TCOOLTHRS, 0x0007D0); // 2000
	this->writeRegister( TCOOLTHRS, 0x0003E8); // 1000
	//this->writeRegister( TCOOLTHRS, 0x001388); // 5000
	//this->writeRegister( TCOOLTHRS, 0x002710); // 10000
	//this->writeRegister( TCOOLTHRS, 0xFFFFFF);


	//this->writeRegister( COOLCONF, SEMIN(0x00) );
	//this->writeRegister( COOLCONF, SGT(0x40) | SEMIN(15)); // -64 least difficult to stall
...
void uStepperDriver::enableStealth()
{
	...
	this->writeRegister( GCONF, EN_PWM_MODE(0) | I_SCALE_ANALOG(1) | ENC_COMMUTATION(0) );
